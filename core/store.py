...
#: Recency column per migrated table; drives the read-only ``last_active_at``
#: hint. Day-only tables fall back to their ``day`` column.
PERSON_MIGRATE_TIME_COLUMNS: dict[str, str] = {
    "relationships": "updated_at",
    "interaction_stats": "updated_at",
    "emotion_events": "ts",
    "affinity_ledger": "created_at",
    "motivation_log": "created_at",
    "sleep_windows": "day",
    "life_events": "ts",
    "life_diary": "day",
}
    ...
    def _person_key_stats_locked(self, persona_id: str, user_id: str) -> tuple[int, str]:
        ...
        with self._lock:
            for table in PERSON_MIGRATE_TABLES:
                column = PERSON_MIGRATE_TIME_COLUMNS.get(table)
                selector = f"MAX({column})" if column else "NULL"
                row = self._conn.execute(
                    f"SELECT COUNT(*) AS n, {selector} AS last FROM {table} "
                    "WHERE persona_id = ? AND user_id = ?",
                    (persona_id, user_id),
                ).fetchone()
